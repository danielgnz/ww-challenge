...
const getAllDates = (startDate, lastRunDate) => {
  ...
  const startYear = parseInt(splitStartDate[0], 10);
  const lastYear = parseInt(lastRunDate.split('-')[0], 10);
  const allDates = [];
...
export const startFetchNi = ({ dispatch, date, allowance }) => {
  ...
  const fetchRequests = dates.map(niStartDate => getNiContributionsForDate(niStartDate));
